...
class OrderItemBronzeETL(TableETL):
    ...
    def extract_upstream(self) -> List[ETLDataset]:
        ...
        if pull_typ == "FULL":
            order_items_data = self.jdbc_conn.read_full_table(
            ...
            order_items_data = self.jdbc_conn.read_incremental_table(
                table_name=qualified_table,
            ...
            raise ValueError(
                f"[OrderItemBronzeETL] Unknown pull_type '{pull_typ}' "
                f"for source table '{table_name}'"
            )

        records_pulled = order_items_data.count()
        ...
        return [order_items_dataset]

    def transform_upstream(self, upstream_datasets: List[ETLDataset]) -> ETLDataset:
        order_items_data = upstream_datasets[0].curr_data
    ...
    def read(self, partition_values: Optional[Dict[str, str]] = None) -> ETLDataset:
        ...
        order_items_data = raw_data.select(
            F.col("order_id"),
            F.col("order_item_id"),
            F.col("product_id"),
            F.col("seller_id"),
