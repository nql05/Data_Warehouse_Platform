...
class CustomerBronzeETL(TableETL):
    ...
    def extract_upstream(self) -> List[ETLDataset]:
        ...
        if pull_typ == "FULL":
            customer_data = self.jdbc_conn.read_full_table(
                table_name=qualified_table,
                partition_column="updated_at",
                num_partitions=16
            )
        elif pull_typ == "INCREMENTAL":
            last_run = self.metadata_conn.get_last_successful_run(self.name)
            customer_data = self.jdbc_conn.read_incremental_table(
                table_name=qualified_table,
            ...
            raise ValueError(
                f"[CustomerBronzeETL] Unknown pull_type '{pull_typ}' "
                f"for source table '{table_name}'"
            )

        records_pulled = customer_data.count()
