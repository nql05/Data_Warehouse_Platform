...
class OrderReviewBronzeETL(TableETL):
    ...
    def extract_upstream(self) -> List[ETLDataset]:
        ...
        if pull_typ == "FULL":
            order_reviews_data = self.jdbc_conn.read_full_table(
            ...
            order_reviews_data = self.jdbc_conn.read_incremental_table(
                table_name=qualified_table,
            ...
            raise ValueError(
                f"[OrderReviewBronzeETL] Unknown pull_type '{pull_typ}' "
                f"for source table '{table_name}'"
            )

        records_pulled = order_reviews_data.count()

        order_reviews_dataset = ETLDataset(
            name=self.name,
            curr_data=order_reviews_data,
        ...
        return [order_reviews_dataset]

    def transform_upstream(self, upstream_datasets: List[ETLDataset]) -> ETLDataset:
        order_reviews_data = upstream_datasets[0].curr_data
    ...
    def read(self, partition_values: Optional[Dict[str, str]] = None) -> ETLDataset:
        ...
        order_reviews_data = raw_data.select(
            F.col("review_id"),
            F.col("order_id"),
            F.col("review_score"),
